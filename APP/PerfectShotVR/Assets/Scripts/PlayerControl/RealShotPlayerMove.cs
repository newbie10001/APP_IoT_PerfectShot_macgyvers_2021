...
public class RealShotPlayerMove : MonoBehaviour
{
    ...
    private void Start()
    {
        StartCoroutine(EnteringShootingLane());
    }
    ...
    public IEnumerator EnteringShootingLane()
    {
        ...
        StartCoroutine(Utility.MoveTo(transform, Enter1[0], 1.0f / 3.0f));
        yield return new WaitForSeconds(3.0f);
        // 회전
        StartCoroutine(Utility.RotateTo(transform, Enter2[1], 1.5f));
        yield return new WaitForSeconds(0.5f);
        StartCoroutine(Utility.MoveTo(transform, Enter3[0], 1.0f / 5.0f));
